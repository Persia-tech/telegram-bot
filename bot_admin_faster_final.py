...
INSTAGRAM_COOKIES_FILE = BASE_DIR / "instagram_cookies.txt"
NODE_RUNTIME_CANDIDATES = ["/usr/local/bin/node", "/usr/bin/node", "node"]
...
def choose_cookie_file(url: str) -> Optional[Path]:
    host = urlparse(url).netloc.lower()
    if "instagram.com" in host:
        return INSTAGRAM_COOKIES_FILE if INSTAGRAM_COOKIES_FILE.exists() else None
    if "youtube.com" in host or "youtu.be" in host:
        return YOUTUBE_COOKIES_FILE if YOUTUBE_COOKIES_FILE.exists() else None
    return None


def choose_node_runtime() -> Optional[str]:
    for candidate in NODE_RUNTIME_CANDIDATES:
...
def build_ydl_opts(url: str, task_dir: Path, progress_hook=None) -> dict:
    ...
    opts = {
        "format": "best" if is_youtube else "bestvideo+bestaudio/best",
    ...
    cookie_file = choose_cookie_file(url)
    if cookie_file:
    ...
    if is_youtube:
        node_runtime = choose_node_runtime()
        if node_runtime:
            opts["js_runtimes"] = {"node": node_runtime}
